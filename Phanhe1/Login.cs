...
namespace Phanhe1
{
    public partial class Login : Form
    {
        ...
        {
            Application.Run(new FORM_TRUONG_PHONG());
        }
        ...
        private void buton_login_Click(object sender, EventArgs e)
        {
            ...
            /*---------------------------------------CHECK ROLE TRƯỞNG PHÒNG (TP) [NEW]------------------------------*/
            else if (check == false)
            {
                Connectionfunction.InitConnection_DBA();
                string ora_tp = "SELECT COUNT(*) FROM DBA_ROLE_PRIVS WHERE UPPER(GRANTEE) = '" + txt_username.Text.ToUpper() + "' AND UPPER(GRANTED_ROLE) = 'TP'";
                Boolean check_tp = Connectionfunction.check_username(ora_tp);
                if (check_tp == true)
                {
                    try
                    {
